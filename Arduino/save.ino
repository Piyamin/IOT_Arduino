...
void loop() {
 ...
 WiFiClient client;
  ...
  http.get("/61105631_IOT/get_finger.php");
//  String statusCode = http.responseStatusCode();
  String response = http.responseBody();
  Serial.print("Number ID: ");
//  Serial.println(statusCode);

  deserializeJson(doc, response);
  String text = doc[0]["id"];
...
//  readnumber(signal);
  ...
  while (!  getFingerprintEnroll(http) );
...
uint8_t getFingerprintEnroll(HttpClient http) {
  int p = -1;
  ...
  if (p == FINGERPRINT_OK) {
    Serial.println("Stored!");
     ...
     http.post("/61105631_IOT/set_finger.php",contentType,postData);
  } else if (p == FINGERPRINT_PACKETRECIEVEERR) {
    Serial.println("Communication error");
    return p;
